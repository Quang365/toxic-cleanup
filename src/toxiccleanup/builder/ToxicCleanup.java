...
public class ToxicCleanup implements Game {
    ...
    private FixedTimer damageTimer;
    ...
    public ToxicCleanup(Dimensions dimensions, Positionable starterTeleporterPosition)
        ...
        final ToxicWorld world = WorldBuilder.fromFile(dimensions, "resources/wasteland.map");
        Machines machines = new MachinesManager();
        machines.spawnTeleporter(starterTeleporterPosition);
        this.guiManager = new GuiManager();
        this.damageTimer = new FixedTimer(DAMAGE_INTERVAL);

        this.gameState = new ToxicCleanupGameState(playerManager, world, machines);
    }
    ...
    public void tick(EngineState engine) {
        ...
        ((ToxicWorld) gameState.getWorld()).tick(engine, gameState);
        ...
        boolean hasToxic = ((ToxicWorld) gameState.getWorld()).allTiles().stream()
                .anyMatch(t -> t instanceof ToxicField && ((ToxicField) t).isToxic());
        ...
        if (damageTimer.isFinished()) {
            ...
            damageTimer = new FixedTimer(DAMAGE_INTERVAL);
        }
    }
    ...
    public List<Renderable> render() {
        ...
        renderables.addAll(((ToxicWorld) gameState.getWorld()).render());
