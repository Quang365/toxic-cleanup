    ...
    public void tick(EngineState state, GameState game) {
        ...
        if (isAlive()) {
            ...
            if (direction != null) {
                ...
                int maxPos = windowSize - tileSize;

                player.setX(Math.clamp(player.getX(), tileSize / 2, maxPos));
                player.setY(Math.clamp(player.getY(), 0, maxPos));
            }
            Positionable pos = player.getPosition();

            List<Tile> tiles = game.getWorld()
                    .tilesAtPosition(pos, state.getDimensions());
